...
export class Stage1Scene implements IScene<typeof name, RoProps, Drawer> {
    ...
    private doDraw(drawer: Drawer, curr: RoProps, next: RoProps) {
        ...
        switch (nextStatusTag) {
            ...
            case "Selecting":
                if (curr.status.tag !== nextStatusTag) {
                    drawer.onSelectionStarted();
                }
                return;
            case "Drawn":
                if (curr.status.tag !== nextStatusTag) {
                    drawer.onDrawn(next.status.remainingFrame);
                }
                return;
            case "Won":
                if (curr.status.tag !== nextStatusTag) {
                    drawer.onWon(next.status.remainingFrame);
                }
                return;
            case "Lost":
                if (curr.status.tag !== nextStatusTag) {
                    drawer.onLost(next.status.remainingFrame);
                }
                return;
        }
    }
