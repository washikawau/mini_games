import { allSqIds, Board, Player, SqId, SqState } from "./Board";
...
test.each([
    ...
    [['o', '-', '-', '-', '-', '-', '-', '-', '-'], ['01', undefined], ['o', 'x', '-', '-', '-', '-', '-', '-', '-']],
    [['o', '-', '-', '-', '-', '-', '-', '-', '-'], ['01', 'o'], ['o', 'o', '-', '-', '-', '-', '-', '-', '-']],
] as [Board, [SqId, Player | undefined], Board][])('.mark()-ok: [%s, %s| %s]', (board, [sqId, state], expected) => {
    // arrange
...
] as [Board, [SqId, Player | undefined], string][])('.mark()-err: [%s, %s| %s]', (board, [sqId, state], expected) => {
    // arrange
    const sut = BoardW.wrap(board);
    // act
...
test.each([
    ...
    [['o', '-', '-', '-', 'x', '-', '-', '-', 'o'], null],
    [['o', '-', '-', 'x', 'x', 'x', 'o', '-', 'o'], 'x'],
] as [Board, Player | undefined][])('.winner(): [%s | %s]', (board, expected) => {
    // arrange
    let sut = BoardW.wrap(board);
    // act
    const a = sut.winner;
