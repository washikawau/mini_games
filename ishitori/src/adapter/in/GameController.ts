...
import { IGameEngine, Input, TouchStart } from "@src/app/port/in/IGameEngine";
import { config } from "@src/common/config";
import { IRepo, assetKeys } from "../IRepo";
import { Stage1SceneController } from "../out/Stage1SceneController";
import { TitleSceneController } from "../out/TitleSceneController";
...
export class GameController {
    ...
    private touchstart: TouchStart | undefined = undefined;
...
function createEngine(repo: IRepo): IGameEngine {
    return createIGameEngine(
        {
            Title: () => new TitleSceneController(repo),
            Stage1: () => new Stage1SceneController(repo),
        },
    );
}
